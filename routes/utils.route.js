...
router.get('/cat/:id', async function (req, res) {
    ...
    for (let i = 0; i < post.length; i++) {
        ...
        post[i].CName = cat_post ? cat_post.CName : '';
        const subcat_post = await subcategoryModel.single2(post[i].SCID);
        if (subcat_post && subcat_post[0] && post[i].SCID !== null) {
...
router.post('/search', async function (req, res) {
    ...
    for (let i = 0; i < result.length; i++) {
        ...
        result[i].CName = cat_post ? cat_post.CName : '';
        const subcat_post = await subcategoryModel.single2(result[i].SCID);
        if (subcat_post && subcat_post[0] && result[i].SCID !== null) {
